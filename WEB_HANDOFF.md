...
2. **状态池模式（莱特纳）** + 子行「池容量 / 低水位阈值 / 重置池」+ 本轮账目（默认 40 / 15，见 §13）
...
```js
{
  ...
  pool:[],                   // 本轮花名册：发到手的 poolSize 个 rank（升序，**含已划掉的**，这样"已掌握 X/N"才看得见）。见 §13.2。【本地】
...
| 本轮花名册 | `pool` | — | 本轮发到手的 `poolSize` 个 rank，**含已划掉的**（见 §13.2） |

- 阈值强制 `poolLow < poolSize`（在设置页改和启动时都会 clamp），否则会一划就补、退化成无限流。
- `poolSize` 上限 = 词库长度（1250）。
- **改容量不会烧掉一轮**：调大 = 往当前花名册**追加**新词到新容量，已掌握的记录**原样保留**（`本轮 100 · 已掌握 53 · 剩余 47` → 改成 150 后是 `本轮 150 · 已掌握 53 · 剩余 97`）；调小 = 装不下已掌握的记录，只能按新容量重开一轮。
- 「重置池」按钮 = 清空花名册，从"最靠前的未掌握词"重新装池。
- 设置页那行显示的就是本轮账目：`本轮 N 词 · 已掌握 X · 剩余 Y（剩余 ≤ 阈值 时淘汰已掌握的、补满到 容量）` + 第二行 `全库已掌握 / 1250`。

### 13.2 机制（`web/template.html`）
**核心是「花名册」语义**：`state.pool` 记的是**本轮发到手的 `poolSize` 个词，含已经划掉的**。已掌握的词**留在花名册上**直到本轮结束——这样「本轮 100 词 · 已掌握 53 · 剩余 47」才看得见。（⚠️ 第一版把已掌握的词每次分页就踢出花名册，导致 `剩余` 恒等于花名册长度、本轮进度完全不可见，已修。）

```
poolRemain()      = 花名册里还没划掉的 rank —— 这就是「剩余」
poolNeedsRefill() = poolMode && poolRemain().length <= poolLow
poolDraw(have,n)  = 从全库 rank 从小到大取 n 个「未掌握且不在册」的词
poolRefill()      = 下一轮：淘汰已掌握 → 顽固词留任 → 注入新词补满 poolSize
poolResize()      = 容量改了：调大只**追加**（保住本轮进度）；调小按新容量重开
ensurePool()      = 自动维护，只在「没池」或「触到低水位」时动手
visible()         = poolMode ? 花名册 && 未划掉 : (removeMarked ? 未划掉 : 全量)
```

- **只有一件事能结束一轮**：`剩余` 掉到 `poolLow`。**在设置页改容量/阈值不算**——第一版把改容量也当成触发，结果输个数字就白白烧掉一轮新词（用户实测踩到：把容量 40 改成 100，53 个已掌握的记录被清掉，显示成"剩余 100/容量 100"）。
- **取词不用游标**：每次注入都从 rank=1 重扫。无须持久化"抽到哪了"，取消划线的词会自然变回可抽——自愈、无脏状态。代价是 O(1250) 扫描，可忽略。
- **写入有护栏**：`poolSet()` 在新旧花名册一致时（例如书末已无词可注）不写 localStorage、返回 false。
- **`ensurePool()` 的调用点**：`paginate()` 开头（此时本轮进行中它什么都不做，**反复进出设置页不会偷偷刷新花名册**）；点击委托的 `mark` 分支里 `poolNeedsRefill()` 为真时；以及设置页开关/改数值/重置池之后。
- `mark` 里那次补池是**全站唯一会因为划词而重排页面的地方**——刻意为之：补池是用户应该看见的"下一轮开始"事件，重排后 `state.anchor` 指到新池首词。平时划词依旧不重排（不会在手指底下抖）。
- 状态池模式下 `removeMarked` 不再另行生效（`visible()` 本来就滤掉了划掉的词）。
- 累计小标（`counters`）照常按 `visible()` 的位置算，所以开池模式时数的是**本轮剩余**的词。

### 13.3 已验证（`scripts/devtest/` 的两个工具，§9.1）
按用户实际用的 100/40 跑了一遍完整轮次：
- 开启 + 容量 100/阈值 40 → `本轮 100 · 已掌握 0 · 剩余 100` ✓
- 划掉 53 个 → `本轮 100 · 已掌握 53 · 剩余 47`，阅读页渲染 47 条 ✓（这正是第一版显示成"剩余 100"的场景）
- **反复进出设置页 3 次** → 账目纹丝不动，花名册没被偷偷刷新 ✓
- 划到第 59 个（剩 41 > 40）→ **不补** ✓；划第 60 个（剩 40 = 阈值）→ **当场开下一轮**：40 个顽固词留任 + 注入 60 个新词 = 100，页面重排、anchor 指到新册首词 ✓
- 容量 100→150（本轮已掌握 58）→ `本轮 150 · 已掌握 58 · 剩余 92`，**没有烧掉一轮** ✓
- 容量 100→60（已掌握 53 装不下）→ 按新容量重开 `本轮 60 · 剩余 60` ✓
- 阈值填 99（≥ 容量 60）→ clamp 到 59 ✓；「重置池」→ 从最靠前的未掌握词重新装池 ✓
- 关掉开关 → 恢复全量 ✓；全程无控制台报错 ✓
