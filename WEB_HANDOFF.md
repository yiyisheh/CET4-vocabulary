...
2. **状态池模式（莱特纳）** + 子行「起始范围 / 推进阈值 / 重置范围」+ 当前范围账目（默认 40 / 15，见 §13）
...
```js
{
  ...
  poolSize:40,               // 起始范围：刚开启时的上界（默认 40）。【本地】
  poolLow:15,                // 推进阈值，同时是推进步长（默认 15，强制 1..poolSize-1）。【本地】
...
| 参数 | state 字段 | 默认 | 含义 |
|---|---|---|---|
| 起始范围 | `poolSize` | **40** | 刚开启时的上界（第 1–40 号） |
| 推进阈值 | `poolLow` | **15** | 剩余 ≤ 此值就推进；**同时也是每次推进的步长** |
| 当前上界 | `poolMax` | — | 整个功能**唯一的状态**：只背 rank ≤ 它、且没划掉的词 |

- 阈值强制 `1 ≤ poolLow < poolSize`（0 会原地踏步死循环）。
- **上界只前进、不后退**，所以改起始范围/阈值永远不会丢进度；「重置范围」才把上界拉回 `poolSize`。
...
ensurePool()      = 幂等：先保证 poolMax ≥ poolSize，再 while(剩余 ≤ 阈值) poolMax += 阈值
```
就一个数 `poolMax`，没有花名册、没有游标、没有"本轮"。好处正是用户要的：**已掌握永远是全库真实总量**（54 就是 54），不会每轮归零；顽固词也不需要特殊照顾——它们没被划掉，自然一直留在范围里。

**为什么不是 `上界 = 起始 + 阈值 × ⌊已掌握 / (起始−阈值)⌋`**（用户最初提的公式，100/40 下即 `100+40×⌊m/60⌋`）：它**每 60 个才推进 40 个**，剩余会一路缩水——m=119 时剩 21、m=179 时剩 1、m=220 时剩 0，而下一次推进要等到 m=240，池子直接饿死。现在改成盯着「剩余」推进，同样满足用户给的 `已掌握 60 → 上界 140`，且剩余恒在 `(阈值, 阈值+起始]` 区间内、不会饿死。
...
按用户实际的 起始 100 / 阈值 40：
- 已掌握 54 → `范围 第 1–100 号 · 已掌握 54 · 剩余 46` ✓（"按 54 个计"）
- 已掌握 59（剩 41 > 40）→ 不推进 ✓；已掌握 **60**（剩 40）→ **推到 140 号** ✓（用户给的例子）
- 已掌握 100 → 180 ✓；已掌握 184 → 260、剩余 76 ✓（用户公式给 220，见 §13.2 为何不采用）
- 旧版 roster `[101..200]` 状态 → 迁移成 `poolMax=200`，旧 `pool` 字段从 localStorage 清掉 ✓
- 「重置范围」→ 回到 1–100 ✓；到书尾自动停在 1250 并显示"已到全书末尾" ✓
- 关掉开关 → 恢复全量 ✓；全程无控制台报错 ✓
